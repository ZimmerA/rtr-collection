...
const float SMAP_FAR = 10.0f;

const glm::vec3 lightPos(1.f, 4.f, -4.f);
...
int main()
{
    ...
    GLFWwindow *window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Environment Shadow Mapping: Color Texture2D", NULL,
                                          NULL);
    ...
    shadowmapVertexShader.attachSourceFromFile("src/advanced/02_ShadowMapping/PointLight/Texture2D/01_ColorTexture/shadowMap.vs");
    Shader shadowmapFragmentShader(GL_FRAGMENT_SHADER);
    shadowmapFragmentShader.attachSourceFromFile("src/advanced/02_ShadowMapping/PointLight/Texture2D/01_ColorTexture/shadowMap.fs");
    ...
    lightingVertexShader.attachSourceFromFile("src/advanced/02_ShadowMapping/PointLight/Texture2D/01_ColorTexture/light.vs");
    Shader lightingFragmentShader(GL_FRAGMENT_SHADER);
    lightingFragmentShader.attachSourceFromFile("src/advanced/02_ShadowMapping/PointLight/Texture2D/01_ColorTexture/light.fs");
    ...
    colorVertexShader.attachSourceFromFile("src/advanced/02_ShadowMapping/PointLight/Texture2D/01_ColorTexture/colorShader.vs");
    Shader colorFragmentShader(GL_FRAGMENT_SHADER);
    colorFragmentShader.attachSourceFromFile("src/advanced/02_ShadowMapping/PointLight/Texture2D/01_ColorTexture/colorShader.fs");
    ...
    Texture shadowMap;
    shadowMap.texImage(0, GL_R32F, SMAP_DIM, SMAP_DIM * 6, 0, GL_RED, GL_FLOAT,
                       NULL);
    ...
    Texture depthBuffer;
    depthBuffer.bind();
    depthBuffer.texImage(0, GL_DEPTH_COMPONENT, SMAP_DIM, SMAP_DIM * 6, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
    ...
    shadowMapBuffer.attachTexture(GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, shadowMap.handle, 0);
    // Matrix setup
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        {
            glViewport(0, SMAP_DIM * i, SMAP_DIM, SMAP_DIM);
            glScissor(0, SMAP_DIM * i, SMAP_DIM, SMAP_DIM);
